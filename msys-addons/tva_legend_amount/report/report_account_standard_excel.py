...
class AccountStandardExcel(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, wizard):
        ...
        # you in all types except 'aged' (Aged balance)
            if wizard.summary:
                sheet = workbook.add_worksheet(report.name)
                _header_sheet(sheet)
            ...
            #MJID: This what we tested
                ...
                def _set_line(line):
                    ...
                    sheet.write(i, 13, _get_data_float(line.get('tva2', '')), currency_format)#mjid
                    sheet.write(i, 14, _get_data_float(line.get('tva2_percent', '')), currency_format)#mjid
                    if line.get('amount_currency', ''):
                        sheet.write(i, 15, _get_data_float(line.get('amount_currency', '')), workbook.add_format({'num_format': line.get('currency')}))
                    sheet.write(i, 16, line.get('matching_number', ''))
...
AccountStandardExcel('report.tva_legend_amount.report_account_standard_excel_amh', 
'account.report.standard.ledger')
