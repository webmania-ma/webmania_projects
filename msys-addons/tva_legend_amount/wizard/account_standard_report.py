...
class AccountStandardLedger(models.TransientModel):
    ...
    def print_excel_report(self):
        self.ensure_one()
        self._compute_data()
        return self.env['report'].get_action(self, 'tva_legend_amount.report_account_standard_excel_amh')
    ...
    def _sql_get_line_for_report(self, type_l, report_object=None):
        ...
        self.env.cr.execute(query, tuple(params))
        res_to_return = self.env.cr.dictfetchall()
#========================================================== MJID ====
#TODO mjid, here we willw computw tva2 and tva2_percent
# chercher sur les compte de 5141 (banque) et 5161 (liquide), puis prix la colonne r(éf:displayed_ref) REFPAY 
#pour trouver les tva de meme ref (Pièces comptables:move_name) REFPCTVA et de compte 4455,  
#et le montant total de facture est, compte 348800 (Pièces comptables:move_name) REFPCFACT ;;; 
#on fait ça avec REFPAY=REFPCTVA=REFPCFACT
        for i in range(0,len(res_to_return)):
            #use self.move_to and self.date_from pour calculer les tva des lignes inclut
            line = res_to_return[i]
            #if line.get('a_code', 'XXXXX').startswith('5141') or line.get('a_code', 'XXXXX').startswith('5161'):
            if line.get('a_code', 'XXXXX').startswith('51'):
                ref_fact = str(line.get('displayed_ref', ''))
                #ref_fact.startswith('FAC/'): vente,ref_fact.startswith('FACTURE/'): Achat
                if len(ref_fact) and ref_fact.startswith('FAC/'):
                    mj_montant_debit_p = line.get('debit', 0)
                    mj_invoice = self.sudo().env['account.invoice'].search([('number','=', ref_fact)], limit=1)
                    if mj_invoice:
                        mj_taxe = mj_invoice.amount_tax
                        mj_total = mj_invoice.amount_total
                        if float(mj_total) != 0:
                            mj_rap = (float(mj_montant_debit_p) / float(mj_total))
                            line['tva2_percent'] = mj_rap * 100.0
                            line['tva2'] = mj_taxe * mj_rap
                        else:
                            line['tva2_percent'] = 0
                            line['tva2'] = 0

                # REFPAY = line.get('displayed_ref', 'elhamdaouiabdelmajid@gmail.com-XXXX')
                # prix_pay = line.get('debit', 0)
                # prix_tva = 0
                # prix_fact = 0
                # for j in range(0,len(res_to_return)):
                #     line2 = res_to_return[j]
                #     if line2.get('a_code', 'XXXXX').startswith('4455') and (
                #         line2.get('move_name', 'elhamdaouiabdelmajid@gmail.com-XXX2') == REFPAY):
                #         #REFPCTVA
                #         prix_tva += line2.get('credit', 0)
                #     if line2.get('a_code', 'XXXXX').startswith('348800') and (
                #         line2.get('move_name', 'elhamdaouiabdelmajid@gmail.com-XXX3') == REFPAY):
                #         #REFPCFACT
                #         prix_fact += line2.get('debit', 0) #usully just one line

                # if prix_fact != 0:
                #     tva2_percent = float(prix_tva) / float(prix_fact)
                #     line['tva2_percent'] = tva2_percent * 100.0
                #     line['tva2'] = prix_pay * tva2_percent
                # else:
                #     line['tva2_percent'] = 0
                #     line['tva2'] = 0
#=====================================================================
